...
func (m *PostModel) Oldest(userID int) ([]*Post, error) {
	stmt := `
        SELECT posts.id, posts.title, posts.content, posts.created, users.username
        FROM posts
        JOIN users ON posts.user_id = users.id
        ORDER BY posts.created ASC LIMIT 10
    `
...
func (m *PostModel) GetByCategoryIDAsc(categoryID, userID int) ([]*Post, error) {
	stmt := `
        SELECT posts.id, posts.title, posts.content, posts.created, users.username
        FROM posts
        JOIN users ON posts.user_id = users.id
...
func (m *PostModel) MostLiked(userID int) ([]*Post, error) {
	stmt := `
        SELECT posts.id, posts.title, posts.content, posts.created, users.username
        FROM posts
        JOIN users ON posts.user_id = users.id
        ...
        GROUP BY posts.id
        ORDER BY COUNT(CASE WHEN post_votes.vote_type = 1 THEN 1 END) DESC
        LIMIT 10
    `
	rows, err := m.DB.Query(stmt)
	if err != nil {
	...
	return m.scanPosts(rows, userID)
}

func (m *PostModel) MostCommented(userID int) ([]*Post, error) {
	stmt := `
        SELECT posts.id, posts.title, posts.content, posts.created, users.username
        FROM posts
        JOIN users ON posts.user_id = users.id
...
func (m *PostModel) scanPosts(rows *sql.Rows, userID int) ([]*Post, error) {
	var posts []*Post
	for rows.Next() {
		post := &Post{}
		err := rows.Scan(&post.ID, &post.Title, &post.Content, &post.Created, &post.Username)
